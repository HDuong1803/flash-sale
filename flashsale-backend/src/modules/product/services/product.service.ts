...
export class ProductService {
  ...
  private async getApprovedMerchant(userId: string) {
    ...
    if (merchant.kycStatus !== 'APPROVED') throw new ForbiddenException('Tài khoản merchant chưa được duyệt')
    return merchant
  }
  ...
  async getInventory(userId: string, productId: string) {
    ...
    if (!inventory) throw new NotFoundException('Không tìm thấy thông tin tồn kho')
